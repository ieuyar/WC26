"""
build_annex_c.py - generate third_place_allocation.csv from FIFA's Annex C.

In the 48-team World Cup, the eight best third-placed teams join the knockout
stage. WHICH group winner each third-placed team faces is not free: FIFA fixes
it in Annex C of the tournament regulations, a lookup table with one row for
every one of the C(12,8) = 495 possible sets of qualifying third-place groups.

This script holds that table verbatim (source: FIFA regulations, Annex C, as
published on the 2026 FIFA World Cup knockout-stage page) and expands it into a
flat CSV the simulator can load.

The eight Round-of-32 slots that receive a third-placed team are the winners of
groups A, B, D, E, G, I, K, L. For each row, the eight values give the GROUP
whose third-placed team is sent to that winner's slot, in the column order:
    1A  1B  1D  1E  1G  1I  1K  1L

Three independent checks run on every row, so a transcription error cannot pass
silently:
  1. the eight allocated groups are exactly the eight qualifying groups;
  2. no winner of group X is paired with the third-placed team of group X;
  3. each allocation respects the published per-match eligibility set
     (e.g. match 74, Winner E, may only draw a third from A/B/C/D/F).
"""
...
# --- Annex C, verbatim -------------------------------------------------------
# Format per line:  "<n>: G;G;G;G;G;G;G;G; 3X;3X;3X;3X;3X;3X;3X;3X"
# First eight tokens  = the eight groups whose third-placed team qualified.
# Last eight tokens   = allocation for winner slots [1A,1B,1D,1E,1G,1I,1K,1L].
ANNEX_C = """
...
# Winner slots that receive a third-placed team, in the table's column order.
SLOT_ORDER = ["A", "B", "D", "E", "G", "I", "K", "L"]

# Per-match eligibility (FIFA Round-of-32 schedule): the third-placed team sent
# to each winner slot must come from one of these groups.
ELIGIBLE = {
    "A": set("CEFHI"),  # Match 79: Winner A
    "B": set("EFGIJ"),  # Match 85: Winner B
    "D": set("BEFIJ"),  # Match 81: Winner D
    "E": set("ABCDF"),  # Match 74: Winner E
    "G": set("AEHIJ"),  # Match 82: Winner G
    "I": set("CDFGH"),  # Match 77: Winner I
    "K": set("DEIJL"),  # Match 87: Winner K
    "L": set("EHIJK"),  # Match 80: Winner L
}


def parse_annex_c():
    """Parse ANNEX_C into {qualifying_groups_str: {slot: third_place_group}}."""
    table = {}
    for line in ANNEX_C.strip().splitlines():
        line = line.strip()
        if not line:
            continue
        _, rest = line.split(":", 1)
        ...
        if len(tokens) != 16:
            raise ValueError(f"Expected 16 tokens, got {len(tokens)}: {line!r}")
        qualifying = tokens[:8]
        allocation = [t.lstrip("3") for t in tokens[8:]]
        key = "".join(sorted(qualifying))
        table[key] = dict(zip(SLOT_ORDER, allocation))
    return table


def validate(table):
    """Run the three integrity checks on every row."""
    # All C(12,8) combinations present, exactly once.
    expected = {"".join(c) for c in combinations("ABCDEFGHIJKL", 8)}
    if set(table) != expected:
        missing = sorted(expected - set(table))
        extra = sorted(set(table) - expected)
        raise ValueError(f"Combination mismatch. Missing: {missing[:5]} "
                          f"Extra: {extra[:5]} (total rows: {len(table)})")

    for key, alloc in table.items():
        qualifying = set(key)
        allocated = set(alloc.values())
        # Check 1: the allocated groups are exactly the qualifying groups.
        if allocated != qualifying:
            raise ValueError(f"Row {key}: allocated {sorted(allocated)} "
                             f"!= qualifying {sorted(qualifying)}")
        for slot, third_group in alloc.items():
            # Check 2: no winner of group X faces the third of group X.
            if slot == third_group:
                raise ValueError(f"Row {key}: winner {slot} paired with own "
                                 f"third-placed team")
            # Check 3: allocation respects per-match eligibility.
            if third_group not in ELIGIBLE[slot]:
                raise ValueError(f"Row {key}: slot {slot} got 3{third_group}, "
                                 f"not in {sorted(ELIGIBLE[slot])}")
    return len(table)
...
def main():
    ...
    table = parse_annex_c()
    n = validate(table)
    print(f"Parsed and validated {n} Annex C combinations (all checks passed).")

    out_path = os.path.join(here, "third_place_allocation.csv")
    with open(out_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["qualified_groups"] + [f"slot_1{s}" for s in SLOT_ORDER])
        for key in sorted(table):
            writer.writerow([key] + [table[key][s] for s in SLOT_ORDER])
